...
Jaydev Madub
Modified to:
	block drag behavior to move 
	out of parent window
...
class DragBehavior(object):
    ...
    def on_touch_move(self, touch):
        ...
        if mode == 'unknown':
            ...
            ud['mode'] = mode
        if mode == 'drag':
            if self.constrain_to_parent_window:
                new_x_pos = self.x + touch.dx
                new_y_pos = self.y + touch.dy                
                if new_x_pos >= self.parent.x and (new_x_pos+self.size[0]) <= (self.parent.width + self.parent.x):
                    self.x += touch.dx
                if new_y_pos >= self.parent.y and (new_y_pos+self.size[1]) <= (self.parent.height + self.parent.y):
                    self.y += touch.dy

                return True
#            print(self.pos, self.parent.height, self.size, touch.dx)
